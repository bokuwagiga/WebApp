...
def get_posts():
    posts = (
        Post.query.options(joinedload(Post.user)).all()
    )
...
def get_comments(post_id):
    post = get_resource_by_id(resource_model=Post, resource_id=post_id, resource_name='Post')
    comments = (
        Comment.query.options(joinedload(Comment.post).joinedload(Post.user)).filter_by(post_id=post_id).all()
    )
...
def home():
    posts = (
        Post.query.options(joinedload(Post.user).joinedload(User.comments)).all()
    )
    posts_list = [post.all_json(['post_id', 'user_id', 'comment_id']) for post in posts]
